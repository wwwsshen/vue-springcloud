  ...
  mutations: {
    ACCOUNT_AUTH_STATUS_CHANGED(state, data) {
      if (data.params.remember) {
        ...
        window.sessionStorage.setItem(USER_PWD, JSON.stringify(data.params))
      }
      state.token = data.token
      state.userInfo = data.staffs[0]
      state.isLogin = true
      window.sessionStorage.setItem(USER_TOKEN, data.token)
      window.sessionStorage.setItem(USER_USERINFOINFO, JSON.stringify(data.staffs[0]))
    ...
    ACCOUNT_LOGOUT_FAILURE(state) {
      ...
      window.sessionStorage.removeItem(USER_USERINFOINFO)
